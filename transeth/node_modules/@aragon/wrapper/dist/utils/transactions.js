...
value:feeDetails.amount.toString()}),applyPretransaction(forwardingTransaction,web3)}async function createDirectTransaction(sender,destination,methodAbiFragment,params,web3){let paramsIncludesTransactionOptions=!1;if("fallback"===methodAbiFragment.type){if(1<params.length)throw new Error("Could not create transaction to fallback function due to too many parameters",params);paramsIncludesTransactionOptions=1===params.length}else methodAbiFragment.inputs.length+1===params.length&&"object"==typeof params[params.length-1]&&(paramsIncludesTransactionOptions=!0);let transactionOptions={};if(paramsIncludesTransactionOptions){const options=params.pop();transactionOptions=_objectSpread(_objectSpread({},transactionOptions),options)}// The direct transaction we eventually want to perform
const directTransaction=_objectSpread(_objectSpread({},transactionOptions),{},{// Options are overwriten by the values below
from:sender,to:destination,data:web3.eth.abi.encodeFunctionCall(methodAbiFragment,params)});// Add any pretransactions specified
return applyPretransaction(directTransaction,web3)}async function createDirectTransactionForApp(sender,app,methodSignature,params,web3){if(!app)throw new Error("Could not create transaction due to missing app artifact");const{proxyAddress:destination}=app;if(!app.abi)throw new Error(`No ABI specified in artifact for ${destination}`);const methodAbiFragment=(0,_abi.findMethodAbiFragment)(app.abi,methodSignature);if(!methodAbiFragment)throw new Error(`${methodSignature} not found on ABI for ${destination}`);return createDirectTransaction(sender,destination,methodAbiFragment,params,web3)}function createForwarderTransactionBuilder(sender,directTransaction,web3){const forwardMethod=new web3.eth.Contract((0,_interfaces.getAbi)("aragon/Forwarder")).methods.forward;return(forwarderAddress,script)=>_objectSpread(_objectSpread({},directTransaction),{},{// Options are overwriten by the values below
