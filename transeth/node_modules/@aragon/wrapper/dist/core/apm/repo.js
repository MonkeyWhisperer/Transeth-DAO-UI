...
versionId:versionId.toString()}}async function fetchRepoContentURI(fileFetcher,contentURI){let{fetchTimeout}=2<arguments.length&&arguments[2]!==void 0?arguments[2]:{};const[provider,location]=contentURI.split(/:(.+)/);if(!provider||!location)throw new Error(`contentURI invalid: ${contentURI}`);else if(!fileFetcher.supportsProvider(provider))throw new Error(`Provider not supported: ${provider}`);let files;try{const timeout=Number.isFinite(fetchTimeout)?fetchTimeout:0,filesFetch=Promise.all([fileFetcher.fetch(provider,location,"manifest.json",{timeout}),fileFetcher.fetch(provider,location,"artifact.json",{timeout})]);files=(await filesFetch).map(JSON.parse)}catch(err){// Fetch failed or timed out
