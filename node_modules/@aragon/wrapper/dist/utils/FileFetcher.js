...
return path.replace(/^[./]+/,"")}function sanitizeUrl(url){return url.startsWith("http://")||url.startsWith("https://")||(url=`http://${url}`),url.endsWith("/")||(url=`${url}/`),url}class FileFetcher{constructor(){let{ipfsGateway=""}=0<arguments.length&&arguments[0]!==void 0?arguments[0]:{};this.providers=new Map([["http"]]),ipfsGateway&&this.providers.set("ipfs",{gateway:sanitizeUrl(ipfsGateway)})}getFullPath(provider,location,path){if(!this.supportsProvider(provider))throw new Error(`Provider not supported: ${provider}`);// When IPFS is the provider, the declared location is a CID
